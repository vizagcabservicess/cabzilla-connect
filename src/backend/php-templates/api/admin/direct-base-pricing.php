<?php
// direct-base-pricing.php - Ultra simplified base vehicle pricing update endpoint
...
error_log($logMessage, 3, __DIR__ . '/../direct-base.log');
...
try {
    ...
    $sql = "SELECT id FROM vehicles WHERE id = ? OR vehicle_id = ?";
    ...
    if ($exists) {
        ...
        $sql = "UPDATE vehicles SET base_price = ?, price_per_km = ?, night_halt_charge = ?, driver_allowance = ? WHERE id = ? OR vehicle_id = ?";
        $stmt = $pdo->prepare($sql);
        $result = $stmt->execute([$basePrice, $pricePerKm, $nightHaltCharge, $driverAllowance, $vehicleId, $vehicleId]);
    } else {
        // Insert new vehicle with default values
        $vehicleName = ucfirst(str_replace(['_', '-'], ' ', $vehicleId));
        $sql = "INSERT INTO vehicles (id, vehicle_id, name, base_price, price_per_km, night_halt_charge, driver_allowance, is_active) VALUES (?, ?, ?, ?, ?, ?, ?, 1)";
        $stmt = $pdo->prepare($sql);
        $result = $stmt->execute([$vehicleId, $vehicleId, $vehicleName, $basePrice, $pricePerKm, $nightHaltCharge, $driverAllowance]);
    }
    
    // Also update outstation_fares table to keep things in sync
    $sql = "SELECT id FROM outstation_fares WHERE vehicle_id = ?";
    $stmt = $pdo->prepare($sql);
    $stmt->execute([$vehicleId]);
    $outstationExists = $stmt->fetchColumn();
    
    if ($outstationExists) {
        $sql = "UPDATE outstation_fares SET base_fare = ?, price_per_km = ?, night_halt_charge = ?, driver_allowance = ? WHERE vehicle_id = ?";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([$basePrice, $pricePerKm, $nightHaltCharge, $driverAllowance, $vehicleId]);
    } else {
        $sql = "INSERT INTO outstation_fares (vehicle_id, base_fare, price_per_km, night_halt_charge, driver_allowance) VALUES (?, ?, ?, ?, ?)";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([$vehicleId, $basePrice, $pricePerKm, $nightHaltCharge, $driverAllowance]);
    }
    ...
    echo json_encode([
        'status' => 'success',
        'message' => 'Base vehicle pricing updated successfully',
        'data' => [
            'vehicleId' => $vehicleId,
            'pricing' => [
                'basePrice' => $basePrice,
    ...
    error_log("Error updating base pricing: " . $e->getMessage());
