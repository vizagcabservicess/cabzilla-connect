  ...
  try {
    ...
    if (tripType === 'airport') {
      ...
      try {
        ...
        if (calculatedFare <= 0) {
          try {
            // Await the promise here to resolve the Type 'Promise<number>' error
            const packagePrice = await getLocalPackagePrice(packageId, cabType.id);
            calculatedFare = packagePrice;
