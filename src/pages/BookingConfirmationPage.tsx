...
import { CheckCircle, Calendar, MapPin, Clock, Car, CreditCard, Download, Share2 } from 'lucide-react';
...
const BookingConfirmationPage = () => {
  ...
  const handleDownloadInvoice = () => {
    toast({
      ...
      description: "Your booking invoice is being prepared for download.",
      duration: 3000,
    });
  };
  ...
  const handleShareBooking = () => {
    if (navigator.share) {
      navigator.share({
        ...
        text: `I've booked a cab with booking ID: ${bookingId}. Pickup: ${booking?.pickupLocation.name}, Dropoff: ${booking?.dropLocation.name}.`,
      ...
      toast({
        ...
        description: "Your browser doesn't support direct sharing. Copy the booking ID and share manually.",
        duration: 3000,
      });
    }
  };
          ...
          <div className="bg-white rounded-xl shadow-card border border-cabGray-100 overflow-hidden animate-fade-in">
            ...
            <div className="p-8">
                ...
                {booking.returnDate && (
                  <div className="space-y-1">
                    ...
                    <div className="flex items-start space-x-2">
                      ...
                      <div>
                        <p className="font-medium text-cabGray-800">
                          {format(new Date(booking.returnDate), 'EEEE, MMMM d, yyyy')}
                        </p>
                        <p className="text-xs text-cabGray-600">
                          {format(new Date(booking.returnDate), 'h:mm a')}
                        </p>
                      </div>
                    </div>
                  </div>
              ...
              <div className="border-t border-cabGray-100 pt-6 mt-6">
                <h3 className="text-lg font-semibold text-cabGray-800 mb-4">Passenger Details</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <p className="text-xs text-cabGray-500">NAME</p>
                    <p className="font-medium text-cabGray-800">{booking.guestDetails.name}</p>
                  </div>
                  <div>
                    <p className="text-xs text-cabGray-500">PHONE</p>
                    <p className="font-medium text-cabGray-800">{booking.guestDetails.phone}</p>
                  </div>
                  <div>
                    <p className="text-xs text-cabGray-500">EMAIL</p>
                    <p className="font-medium text-cabGray-800">{booking.guestDetails.email}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-8 flex flex-col md:flex-row gap-4 justify-center">
                ...
                <Button onClick={handleShareBooking} className="flex items-center" variant="outline">
