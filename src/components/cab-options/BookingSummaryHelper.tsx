...
export const BookingSummaryHelper: React.FC<BookingSummaryHelperProps> = ({
    ...
    const handleFareSourceUpdate = (event: Event) => {
      ...
      if (customEvent.detail && customEvent.detail.cabId && customEvent.detail.source) {
        ...
        if (selectedCabId && cabId === selectedCabId.toLowerCase().replace(/\s+/g, '_')) {
          console.log(`BookingSummaryHelper: Tracked fare source for ${cabId} is ${source} via ${apiUrl}`);
  ...
  useEffect(() => {
    ...
    if (tripType === 'local' && hourlyPackage) {
      ...
      // Directly fetch from the database API - EXACTLY matching the CabList order
      const fetchFareFromDatabase = async () => {
        // Create an array of fetch attempts to try in order
        const fetchAttempts = [
          ...
          // Last resort: use direct calculation
          async () => {
            ...
            // Fallback to 'sedan' if not found in base prices
      ...
      fetchFareFromDatabase().then(result => {
        if (result && result.price > 0) {
          ...
          if (Math.abs(result.price - totalPrice) > 10) {
            console.log(`BookingSummaryHelper: Database price (${result.price}) differs from component price (${totalPrice}), broadcasting update`);
