...
export function LocalFareManagement() {
  ...
  const [fareData, setFareData] = useState<LocalFare | null>(null);
  ...
  const loadFareData = async (vehicleId: string, forceRefresh = false) => {
    ...
    try {
      ...
      const data = await fareService.getLocalFaresForVehicle(vehicleId);
      console.log('Loaded fare data:', data);
      setFareData(data);
    } catch (error: any) {
      console.error('Error loading local fare data:', error);
      setError(error);
  ...
  useEffect(() => {
    const handleFareUpdated = (event: Event) => {
      ...
      console.log('Local fares updated event:', detail);
    ...
    const handleFaresSynced = () => {
      console.log('Local fares synced event received');
      if (selectedVehicleId) {
        loadFareData(selectedVehicleId, true);
      }
      refetchVehicles();
    ...
    window.addEventListener('local-fares-synced', handleFaresSynced);
    
    return () => {
      window.removeEventListener('local-fares-updated', handleFareUpdated);
      window.removeEventListener('local-fares-synced', handleFaresSynced);
    };
  ...
  // Handle sync all local fares
  const handleSyncLocalFares = async () => {
    setIsSyncing(true);
    ...
    try {
      ...
      toast.success(
        `Local fares synced successfully: ${response.stats.created} created, ${response.stats.updated} updated`
      );
  ...
  return (
    <Card className="w-full">
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        <div>
          <CardTitle>Local Package Fare Management</CardTitle>
          <CardDescription>
            Configure the fares for local package services
          </CardDescription>
        </div>
      ...
      <CardContent className="pt-6">
        ...
        {error ? (
          <FareUpdateError 
            ...
            isAdmin={true}
            title="Error Loading Local Fares"
            description="There was a problem loading the local fare data."
            fixDatabaseHandler={handleFixDatabase}
          />
        ) : isLoading ? (
          ...
          <div className="text-center py-8 text-muted-foreground">
            Please select a vehicle to manage its local package fares
          </div>
        )}
      </CardContent>
