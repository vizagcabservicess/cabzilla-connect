...
export function MobileTabSelector({ selectedTab, onTabChange }: MobileTabSelectorProps) {
  ...
  return (
    <div className="flex rounded-full p-1 bg-gray-100 overflow-hidden relative">
      {tabs.map((tab) => (
        <button
          key={tab.id}
          onClick={() => handleTabChange(tab.id)}
          className={`relative z-10 flex-1 py-2.5 px-3 text-sm text-center rounded-full transition-colors duration-200 ${
            activeTab === tab.id 
      ...
      <motion.div
        className="absolute top-1 left-1 bottom-1 rounded-full bg-white border border-gray-200 shadow-sm"
        initial={false}
        animate={{
